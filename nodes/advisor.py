...
def advisor(state:AgentState)-> AgentState:
    ...
    try: 
        ...
        prompt_template = PromptTemplate(
            input_variables=["idea_analysis","market_analysis", "competition_analysis", "risk_assessment", "swot_analysis"],
            template=template,
            partial_variables={"format_instructions": parser.get_format_instructions()}
        )
        chain=prompt_template | chat_model | parser
        response=chain.invoke({"idea_analysis":state["idea_analysis"],"market_analysis":state["market_analysis"], "competition_analysis":state["competition_analysis"], "risk_assessment":state["risk_assessment"], "swot_analysis":state["swot_analysis"]})
